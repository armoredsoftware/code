...
--separate by ;;'s
separateIntoCommands :: [String]-> [String] -> [[String]]
separateIntoCommands [] xs = [xs ++ []]
separateIntoCommands (x : xs) cur | (isSuffixOf ";;" x) = (cur ++ [x]) : (separateIntoCommands xs [])
--now get 'parts'
                                  | otherwise = separateIntoCommands xs (cur ++ [x])
...
--code repitition, but I'm not sure how to do it another way yet
