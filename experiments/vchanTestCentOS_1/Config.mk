...
# Copied from xen-4.2.2/tools/Config.mk and modified.

ifeq ($(filter /%,$(EXP_ROOT)),)
$(error EXP_ROOT must be absolute)
endif

# fallback for older make
#realpath = $(wildcard $(foreach file,$(1),$(shell cd -P $(dir $(file)) && echo "$$PWD/$(notdir $(file))")))

-include $(EXP_ROOT)/.config
...
include $(EXP_ROOT)/config/$(XEN_OS).mk
include $(EXP_ROOT)/config/$(XEN_TARGET_ARCH).mk
...
QEMU_UPSTREAM_URL ?= $(EXP_ROOT)/tools/qemu-xen
...
# CONFIG_QEMU ?= `pwd`/$(EXP_ROOT)/../qemu-xen.git
# CONFIG_QEMU ?= $(QEMU_REMOTE)
CONFIG_QEMU   ?= $(EXP_ROOT)/tools/qemu-xen-traditional
